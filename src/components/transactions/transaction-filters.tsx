...
function hrefFrom(next: TransactionFilters, today: string): string {
	const query = buildTransactionSearchParams(
		next,
		toUtcMidnight(today),
	).toString();
		...
		<Collapsible className="overflow-hidden rounded-md border">
          ...
          ghost variant that fight a full-width header bar. */}
			...
			>
				{t("filters.title")}
				{activeCount > 0 && (
					<Badge>{t("filters.active", { count: activeCount })}</Badge>
				)}
				<ChevronDown
					aria-hidden="true"
			...
			<CollapsibleContent>
				<div className="grid grid-cols-12 gap-4 p-4">
                    ...
                    the `null` "All" entry. */}
							...
							<SelectContent>
								{/* Bold, like the All row in the category and card fields:
                    the entry that undoes a filter reads differently from the
                    ones that apply one. */}
						...
						>
							...
							<SelectContent>
								{TRANSACTION_SHOW_VALUES.map((show) => (
									// `"all"` is this select's own unfiltered entry, so it is
									// bold for the same reason the other two are.
									<SelectItem
										key={show}
										value={show}
										className={cn(
