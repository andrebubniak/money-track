...
import { createInstallmentSchema, type InstallmentValues } from "@/lib/validations/installment";
...
type InstallmentFormProps = {
  ...
  defaultValues: Omit<InstallmentValues, "occurrencesCount"> & { occurrencesCount: number | "" };
...
export function InstallmentForm({
  ...
  const schema = useMemo(() => createInstallmentSchema(tValidation), [tValidation]);
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-12 flex flex-col gap-2 lg:col-span-3">
        ...
        <Input
          ...
          aria-describedby={errors.occurrencesCount ? "occurrencesCount-error" : undefined}
          className="lg:h-11 lg:text-base"
          defaultValue={defaultValues.occurrencesCount}
          {...register("occurrencesCount")}
        />
        {errors.occurrencesCount && (
          <p id="occurrencesCount-error" className="text-sm text-destructive">
            {errors.occurrencesCount.message}
          </p>
        )}
      </div>

      {preview && <p className="col-span-12 text-sm text-muted-foreground">{preview}</p>}
